        ...
        // 1. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
        {
            ...
            if (ImGui::Button(xorstr("load"), ImVec2(177, 21))) {
                ...
                if (Authorization()) {
                    ...
                    EndFile += xorstr(".dll");
                    DeleteUrlCacheEntry(skCrypt(L"https://google.com/cheat.dll")); // ������� ���, ������ ������ �� ���� DLL
                    if (URLDownloadToFileA(0, skCrypt("https://google.com/cheat.dll"), EndFile.c_str(), 0, 0) != S_OK) { InternetCloseHandle(hInternet); InternetCloseHandle(hUrl); return 0; }      // ��������� dll
                    HANDLE hProc = GetProcessByName(skCrypt(L"javaw.exe")); //���� �������
