...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, ImColor(188, 188, 188, 50));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(8, 8), check_bb.Max - ImVec2(8, 8), ImColor(50, 50, 50));
    ...
    else if (*v)
    {
        ...
        ImGui::GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(8, 8), check_bb.Max - ImVec2(8, 8), ImColor(195, 141, 0));
    }

    if (g.LogEnabled)
        LogRenderedText(&total_bb.Min, *v ? "[x]" : "[ ]");
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + 0.1), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        ImGui::GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(22, 22, 22));
        ImGui::GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, ImColor(188, 188, 188, 50));
    }
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        ...
                        draw_window->DrawList->AddRect(rect.Min, rect.Max, ImColor(188, 188, 188, 50));
                }
